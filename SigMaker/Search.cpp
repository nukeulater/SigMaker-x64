...
{
    int iCount = 0;
    ea_t dwAddress = find_binary(inf_get_min_ea(), inf_get_max_ea(), strSig.c_str(), 16, SEARCH_DOWN);
	...
	do
	{
		if (bSkipOut == true && iCount >= 2)
			return iCount;
		iCount++;
		dwAddress = find_binary(dwAddress + 1, inf_get_max_ea(), strSig.c_str(), 16, SEARCH_DOWN);
...
void SearchForSigs( const qstring& strSig )
{
	...
	dwAddress = find_binary(inf_get_min_ea(), inf_get_max_ea(), strSig.c_str(), 16, SEARCH_DOWN);
